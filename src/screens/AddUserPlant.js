...
const uploadImageToFreeImageHost = async (imageUri) => {
  ...
  try {
    ...
    if (data.status_code === 200) {
      return data.image.display_url; // Returns the public image URL
    } else {
  ...
  if (!name || !family  || !waterCycles || !sunlight || !temperature || !description) {
    ...
    // Save to Firebase
    try {
      await setDoc(doc(firestore, "user_plants",name), {
        uid:plantId,
        name,
        family,
        waterCycles,
  ...
  return (
    ...
    <ScrollView className="flex-1 bg-white">
      ...
      <View className="bg-black h-[80vh] flex-1 px-6 pt-10 pb-16 ">
 ...
 <View className="flex-row z-[999] justify-around items-center py-4 bg-black">
        <FontAwesome onPress={()=>{ navigation.navigate("MainScreen");  }} name="home" size={24} color="white" />
        <FontAwesome  onPress={()=>{ navigation.navigate("UserPlants");  }} name="heart" size={24} color="white" />
        <FontAwesome onPress={()=>{ navigation.navigate("CartScreen");  }}  name="shopping-cart" size={24} color="white" />
        <FontAwesome onPress={()=>{ navigation.navigate("ProfileScreen");  }}  name="user" size={24} color="white" />
      </View>
</View>
  );
}
